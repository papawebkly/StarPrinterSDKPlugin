...
/*!
 *  Portable Printer is not supported
 */
- (NSData *)getImageDataForPrinting:(BOOL)compressionEnable
{
	...
	// Converts the image to a Monochrome image using a Steinbert Dithering algorithm.
    // This call can be removed but it that will also remove any dithering.
    ...
    memset(constructedBytes, 0x00, 3 + mWidth);
...
/*!
 *  TSP100 series is not supported
 */
- (NSData *)getGraphicsDataForPrinting:(BOOL)compressionEnable
{
    ...
    u_int8_t *constructedBytes = (u_int8_t *)malloc(9 + mWidth);
    if (constructedBytes == NULL) {
        free(pixels);
        [data release];
        
        return nil;
    }
    memset(constructedBytes, 0x00, 9 + mWidth);
    
    constructedBytes[0] = 0x1b; // ESC
    constructedBytes[1] = 0x1d; // GS
    constructedBytes[2] = 0x53; // 'S'
    constructedBytes[3] = 1;    // m
    constructedBytes[4] = 0x00; // xL (dummy)
    constructedBytes[5] = 0x00; // xH (dummy)
    constructedBytes[6] = 0x01; // yL (1 line)
    constructedBytes[7] = 0x00; // yH
    constructedBytes[8] = 0;    // n  (black)
    ...
    for (int y = 0; y < height; y++)
    {
        ...
        for (int x = 0; x < mWidth; x++)
        {
            ...
            constructedBytes[9 + x] = constructedByte;
        }
        ...
        if (compressionEnable)
        {
            while (work)
            {
                ...
                if (constructedBytes[9 + work] != 0x00)
                {
                    work++;
                    break;
                }
        ...
        if (work)
        {
            while (blank >= 0xff)
            {
                u_int8_t blankBytes[] = {0x1b, 'I', 0xff};
                [data appendBytes:blankBytes length:3];
            ...
            if (blank)
            {
                u_int8_t blankBytes[] = {0x1b, 'I', 0x00};
                blankBytes[2] = blank;
            ...
            constructedBytes[4] = (u_int8_t)(work % 256);
            constructedBytes[5] = (u_int8_t)(work / 256);
            
            [data appendBytes:constructedBytes length:9 + work];
        }
        else
        {
            blank++;
    ...
    u_int8_t cutBytes[] = {0x1b, 'd', 0x02};
    [data appendBytes:cutBytes length:3];
    
    free(constructedBytes);
    
    free(pixels);
    
    imageData = [NSData dataWithBytes:data.mutableBytes length:data.length];
...
- (NSData *)getImageMiniDataForPrinting:(BOOL)compressionEnable pageModeEnable:(BOOL)pageModeEnable
{
    ...
    if (pageModeEnable)
    {
        ...
        [someData appendBytes:beginingBytes length:2 + 2 + 10];
    }
    else
    {
        u_int8_t beginingBytes[] = {0x1b, 0x40};
    ...
    while (totalRowCount < height)
    {
        ...
        if (compressionEnable)
        {
            NSString *portSettings = @"Portable;ESCPOS";

            command = [SMPort compressRasterData:byteWidth :24 :data :portSettings];
            // command = [SMPort generateBitImageCommand:byteWidth :24 :data :portSettings]; // Deprecated API
        }
	...
	[someData appendBytes:endingBytes length:1];
