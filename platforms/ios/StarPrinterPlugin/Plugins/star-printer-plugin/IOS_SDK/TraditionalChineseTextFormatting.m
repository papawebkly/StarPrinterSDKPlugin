//
//  TraditionalChineseTextFormatting.m
//  IOS_SDK
//
...
@implementation TraditionalChineseTextFormatting
...
- (void)viewDidLoad
{
    ...
    uitextview_texttoprint.text = @"此功能與上面所定義的打印機裝修發送原始文本。\n";
}
...
- (IBAction)PrintText
{
	...
	NSData *textNSData = [uitextview_texttoprint.text dataUsingEncoding:CFStringConvertEncodingToNSStringEncoding(kCFStringEncodingBig5)];
    
    unsigned char *textData = (unsigned char *)malloc(textNSData.length);
    [textNSData getBytes:textData length:textNSData.length];
    
	[PrinterFunctions PrintCHTTextWithPortname:portName
